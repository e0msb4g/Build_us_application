...
「見栄え」、「アプリの使いやすさ」を最も念頭に置きつつ制作いたしました。「見栄え」の面では、ストレスを感じにくい薄めの色合いを基調とし、ページ遷移や投稿などを行うリンクには青や赤などの原色を使用することで、ユーザーが直感的に次に何をすれば良いかを分かりやすくしております。「使いやすさ」の面では、ログイン後すぐにマイページのビューを用意し、以前気になった掲示板をすぐに閲覧できるようにしております。そして、アプリを閲覧しながら、実際に現地に向かうこともできるように、詳細ページではGoogleMapを導入し、レスポンジブウェブデザインにも対応いたしました。
...
### 「ユーザーの発信する情報の信憑性が高くなりづらい」

メールアドレスを取得さえすれば、誰にでも投稿、コメントできる一方で、ユーザーに対し、住所などの個人情報を求めていないため、発信情報に信憑性を持たせることができておりません。ですが、ユーザーに個人情報を求めることで、多くの方に情報を発信、取得していただくことが難しいと考えましたので、今後は以下の機能を実装予定です。
